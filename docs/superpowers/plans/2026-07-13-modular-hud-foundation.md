...
`UserSettingsJson` は `JsonException` と `NotSupportedException` だけを既定値へフォールバックし、Serialize前にも構造を正規化する。null／空白のDefaultHudIdとnull配列だけを既定値へ補完し、未知の非空IDと空配列は保持する。
...
internal static class UserSettingsJson
{
    ...
    private static UserSettings NormalizeStructure(UserSettings settings)
    {
        ...
        settings.DefaultHudId = string.IsNullOrWhiteSpace(settings.DefaultHudId)
            ? BuiltInHudIds.Music
        ...
        settings.AdditionalProperties ??= new Dictionary<string, JsonElement>(StringComparer.Ordinal);
        return settings;
    }
}
```
...
`NavigateToAsync`、`DisableAsync`、`ShutdownAsync` は同じSemaphoreSlimを使う。`OnModulePresentationInvalidated` は遷移中ならboolの保留フラグだけを立て、遷移外ならPresentationChangedを通知する。フラグは短いlockで保護する。
...
`RefreshHudPresentation` は毎回CurrentModule、CurrentHudId、PresentationStateを取得し、CurrentModuleへGetViewとGetPreferredSizeを問い合わせる。Collapsedなら共通定数200x2でコンテンツを閉じ、その他では全画面抑制を確認してViewを表示する。PinnedのMouseLeaveは `router.CollapseFromPointerLeave()` に委譲する。
