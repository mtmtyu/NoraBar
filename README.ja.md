...
- **設定画面**
  - デザインスタイル、プログレスバー表示、同期歌詞表示、テキストの自動スクロール、自動起動、表示言語、HUD の表示位置を切り替えられます。
  - フルスクリーン時に展開を無効化し、HUDが全画面アプリを隠さないように設定できます。
...
NoraBar はサードパーティのコンポーネント / ライブラリ / サービスとして CSCore、LRCLIB、Material.Icons.WPF を使用しています。ライセンス情報はアプリ内の設定画面から確認できます。
