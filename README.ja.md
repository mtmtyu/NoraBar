...
画面のいちばん上に、ふっと現れて切り替えられる小さな HUD。

NoraBar は Windows デスクトップ向けの WPF アプリケーションです。普段は画面上端に 2px の細いエリアとして静かに待機し、マウスを近づけると開きます。音楽操作に集中した音楽 HUD、必要な情報を一覧できるホーム HUD、アプリやファイルをすぐ開けるランチャー HUD を、作業場所の上端から離れずに切り替えられます。
...
- **テキストの自動スクロール**
  - 曲名やアーティスト名が長くて表示しきれない場合に、自動でスクロール表示します。
  - スクロール無効、ホバー時のみスクロール、常にスクロールの3つのモードをサポートします。

- **同期歌詞表示**
  - LRCLIB から取得できる場合、再生中の曲に合わせて同期歌詞を表示します。
  - 現在の歌詞行を HUD 上に表示します。
  - 設定画面から表示のオン / オフを切り替えられます。

- **フルスクリーン時の展開無効化**
  - ゲームや動画のフルスクリーン再生中などに、HUDが意図せず展開されるのを防ぎます。
...
- **ホーム HUD**
  - ローカル時刻、日付、設定可能な最大3つのワールドクロック、再生中メディア、再生操作をまとめて表示します。
  - 時計、ワールドクロック、メディアのウィジェットを追加・削除・並べ替え・サイズ調整できる動的レイアウトを使用します。
  - システム準拠、12時間、24時間の時刻形式に対応します。

- **ランチャー HUD**
  - Win32 アプリ、Store アプリ、ファイル、フォルダー、HTTP/HTTPS URL を順序付きページとグループで整理できます。
  - 登録済み／インストール済みアプリの検索、既存ウィンドウへの切替、ローカル Smart 候補、コンテキスト Rules に対応します。
  - 使用履歴は端末内だけに保存し、任意のグローバルショートカットと設定画面内の構造編集を提供します。

- **切り替え可能な HUD ナビゲーション**
  - 右側のアイコンレールと、上側のアイコン＋名前タブを設定で選べます。
  - ナビゲーション領域のクリックまたはスクロールで HUD を切り替えます。
  - 設定から HUD の有効化、無効化、並べ替え、起動時 HUD の指定ができます。

- **設定画面**
  - 各 HUD をライブプレビューしながら設定でき、プログレスバー表示、同期歌詞表示、テキストの自動スクロール、自動起動、表示言語、HUD の表示位置も切り替えられます。
  - 起動時の自動アップデート確認、手動アップデート確認、ビジュアライザー再起動機能、サードパーティライセンス表示も備えています。
...
一部の機能にはインターネット接続が必要です。
- 同期歌詞表示: LRCLIB にアクセスします。
- アップデート確認: GitHub Releases にアクセスします。
...
`docs/wiki/` 配下の Markdown は、`master` ブランチへの push 時に GitHub Wiki へ同期されます。
...
NoraBar はサードパーティのライブラリ / サービスとして CSCore、Material.Icons.WPF、および LRCLIB を使用しています。ライセンス情報はアプリ内の設定画面から確認できます。
